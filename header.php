<!DOCTYPE html>
<html lang="en">

    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>Anotando</title>
        <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.min.css">
        <link rel="preconnect" href="https://fonts.googleapis.com">
        <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
        <link
            href="https://fonts.googleapis.com/css2?family=Jua&family=Source+Sans+3:ital,wght@0,600;0,900;1,300&display=swap"
            rel="stylesheet">
        <link rel="stylesheet" href="css/style.css">



    </head>

    <header>
        <div class="nome-pagina">
            <h2>Dashboard</h2>
        </div>
        <div class="logo-anotando">
            <img src="assets/img/logos/Anotando_claro (1).png" alt="">
        </div>
    </header>

    <sidebar>
        <!-- sidebar aqui -->
        <a href="pfp.php"><img class="icons-sideBar" src="assets/img/icons/perfil.png" alt=""></a>
        <a href="dashboard.php"><img class="icons-sideBar" src="assets/img/icons/home.png" alt=""></a>
        <a href="#"><img class="icons-sideBar" src="assets/img/icons/config.png" alt=""></a>
        <a href="#"><img class="icons-sideBar" src="assets/img/icons/telefone.png" alt=""></a>
    </sidebar>

    <script src="https://code.jquery.com/jquery-3.3.1.slim.min.js"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.14.7/umd/popper.min.js"></script>
    <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/js/bootstrap.min.js"></script>


</html>